...
export const registerManager = async ({
  name, email, password, hub_id
} : RegisterParams) => {
  try {
    const { data } = await clientApi.room.register_manager({name, email, password, hub_id});
...
export const getUser = async ({user_id, home_id} : UserDataParams) => {
  try {
    const { data } = await clientApi.room.getUser({user_id, home_id});
    if (data.statusCode === 200 || data.statusCode === 201) {
      return data.data;
    }
    throw Error(data.responseInfo.message);
